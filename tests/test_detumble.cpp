...
struct TestStep
{
  ...
  TestStep(const glm::vec3 &gyro, bool wheelsHealthy = true, float wheelSpeedRadS = 0.0f) : imu{gyro, glm::vec3(0.0f)}
  {
    for (int w = 0; w < NUM_WHEELS; w++)
      wheelTelemetry[w] = {wheelSpeedRadS, wheelsHealthy};
  }

  void runOn(ADCS &adcs, float dt) const
  {
...
int main()
{
  // High initial rate, commanded mode TARGET -> auto-enters DETUMBLE.
  {
    ...
    TestStep(highRate).runOn(adcs, 0.05f);
  ...
  // Rate below entry threshold the whole time -> no false trigger.
  {
    ...
    TestStep step(lowRate);
    for (int i = 0; i < 100; i++)
      step.runOn(adcs, 0.05f);
  ...
  // confirms an aggressive slew genuinely pins body rate near that cap.
  {
    ...
    TestStep(slewCapRate).runOn(adcs, 0.05f);
  ...
  // to SUN_POINTING.
  {
    ...
    // Auto-enter.
    TestStep(glm::vec3(1.0f, 1.0f, 1.0f)).runOn(adcs, 0.05f);
    ...
    TestStep(glm::vec3(0.01f, 0.0f, 0.0f)).runOn(adcs, 0.05f);
  ...
  // even with rate low the whole time.
  {
    ...
    TestStep step(glm::vec3(0.01f, 0.0f, 0.0f)); // already below exit threshold
    for (int i = 0; i < 100; i++)
      step.runOn(adcs, 0.05f);
  ...
  // AUTO actuator selection: wheels at 0% saturation -> REACTION_WHEELS.
  {
    ...
    TestStep(glm::vec3(0.3f, 0.0f, 0.0f), /*wheelsHealthy=*/true, /*wheelSpeedRadS=*/0.0f).runOn(adcs, 0.05f);
  ...
  // AUTO actuator selection: wheels above the saturation budget -> MAGNETORQUERS_BDOT.
  {
    ...
    TestStep(glm::vec3(0.3f, 0.0f, 0.0f), /*wheelsHealthy=*/true, highSpeed).runOn(adcs, 0.05f);
