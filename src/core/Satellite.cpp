...
float Satellite::updatePower(float dt)
{
  ...
  for (int i = 0; i < NUM_WHEELS; i++)
    drawW += Config::WHEEL_IDLE_POWER_W +
             std::abs(wheels[i]->commandedTorque * wheels[i]->currentSpeed) / Config::WHEEL_MOTOR_EFFICIENCY;
  for (int i = 0; i < NUM_TORQUERS; i++)
    drawW += Config::TORQUER_IDLE_POWER_W +
             std::abs(magnetorquers[i]->commandedDipoleMoment) * Config::TORQUER_POWER_PER_AM2_W;
